...
export default function FABStockReceiveForm() {
  ...
  async function loadProducts(searchValue = "") {
    ...
    try {
      ...
      const response = await fetch(
        `/api/inventory/stock-received/fab-stock?${params.toString()}`,
        {
          cache: "no-store",
        }
      );
      ...
      setNewProductEnabled(Boolean(data.newProduct?.enabled ?? true));
  ...
  async function saveStock() {
    ...
    try {
      ...
      const response = await fetch(
        "/api/inventory/stock-received/fab-stock",
        {
          method: "POST",
          headers: {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm font-bold text-slate-500">
          {baseName
            ? `${baseName} — FAB Doha Stock Received`
            : "Receive stock into the FAB Doha Stock inventory base."}
        </p>
      </div>
          ...
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm font-bold text-amber-800">
                FAB Doha Stock — SKU, Doha Price, image and opening quantity
              </p>
            </div>
