...
async function searchProducts({
  ...
  if (attachmentsField) {
    ...
    do {
      ...
      if (offset) {
        stockParams.set("offset", offset);
      }

      const stockResponse = await fetch(
        airtableUrl(baseId, stockReceivedTable.name, stockParams),
        {
      ...
      if (!stockResponse.ok) {
        throw new Error(
          stockData?.error?.message ||
            ...
            "Unable to load Stock Received attachments"
        );
      }

      for (const record of stockData.records || []) {
        const linkedProductIds = Array.isArray(
          record.fields?.[linkField.name]
        )
          ? record.fields[linkField.name]
          : [];

        const attachments = Array.isArray(
          record.fields?.[attachmentsField.name]
        )
          ? record.fields[attachmentsField.name]
          : [];
      ...
      offset = String(stockData.offset || "");
    } while (offset);
  }
  ...
  try {
    ...
    if (!isDQBase) {
      return NextResponse.json(
        {
          success: false,
          message: "Select the i5Q/DQ base to use DQ Stock Receive",
        },
        { status: 400 }
      );
    }
    ...
    const products = await searchProducts({
      ...
      stockReceivedTable: setup.stockReceivedTable,
      linkField: setup.linkField,
      attachmentsField: setup.attachmentsField,
...
async function createNewProductAndReceiveStock({
  ...
  if (!imageUrl) {
    throw new Error("Product image is required for a new DQ SKU");
  }
  ...
  if (setup.attachmentsField && imageUrl) {
    stockFields[setup.attachmentsField.name] = [
      { url: imageUrl },
    ];
  }
