...
export default function UaePublicStockPage() {
  ...
  async function loadProducts() {
    try {
      ...
      const response = await fetch("/api/public/stock/uae", { cache: "no-store" });
      ...
      <header className="bg-slate-950 px-4 py-5 text-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3">
          <div>
            ...
            <h1 className="text-2xl font-black">UAE Stock</h1>
          ...
          <>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
              {filteredProducts.slice(0, visibleCount).map((product) => (
                <ProductCard key={product.id} product={product} onImageClick={setSelectedProduct} currency="AED" whatsAppNumber="" />
              ))}
            </div>
